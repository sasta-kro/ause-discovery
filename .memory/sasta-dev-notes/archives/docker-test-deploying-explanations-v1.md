...
## 1. Create local deployment configuration
...
Edit `.env.local-test` and change:
...
The remaining development defaults are suitable for a local-only test. Do not use them in production.

## 2. Build and bootstrap the stack
...
The first build may take a while because unused Docker build caches were cleaned.
...
Both health requests should return JSON with a successful status.

Open:

- Application: [http://localhost:8090/ause-discovery/](http://localhost:8090/ause-discovery/)
- Administrator login: [http://localhost:8090/ause-discovery/admin/login](http://localhost:8090/ause-discovery/admin/login)

## 5. Suggested manual test
...
9. Optionally test CSV or XLSX import.

Browser acceptance can also run against the stack:

```sh
AUSE_E2E_BASE_URL=http://localhost:8090/ause-discovery/ make test-e2e
```

Without a configured Project fixture, two tests pass and the Artifact-specific test is skipped.

## 6. Stop or delete the test deployment
...
Permanently remove the isolated test data:
