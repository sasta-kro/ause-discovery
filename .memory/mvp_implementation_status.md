...
- Correction verification passed: backend package tests for `pagecursor`, People, Projects, and the HTTP server covering q/status filtering, two-page cursor pagination without duplicates, deterministic ties, final-page behavior, and malformed-cursor `400` responses; `gofmt` clean; and an authenticated HTTP smoke against a disposable administrator and database proving Projects and People `next_cursor` round trips at small limits, uppercase `q` filtering, and malformed-cursor rejection on both lists.
