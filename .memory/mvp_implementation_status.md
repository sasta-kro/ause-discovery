...
- Local CI parity targets (`check-source`, `check-integration`, `generate-check`, `check-images`, `check-compose`, `vuln-report`, `check-action-pins`, `check-action-pins-test`) run the same checks; `generate-openapi-typescript` now invokes the pure-JavaScript generator shim in the pinned Node container instead of containerized pnpm.
