...
## Corpus extraction status (2026-09-09)

All 145 reviewed corpus projects re-read in a discovery pass (29 chunks plus single-file retries): free-form taxonomy suggestions with used-vs-cited evidence, grounded generated abstracts for 32 previously abstractless documents, and classification re-examination. Result: `output/reviewed-import.csv` now has 141 rows (88 CS / 53 IT), only 4 drops remain (1651, 1661, 1813, 1904 print no semester or year anywhere). The taxonomy suggestion report is `tools/ausesp-data-extractor/output/discovery-report.md`: top technology candidates by genuine use are javascript (22), bootstrap (18), csharp (14), aws (13), tensorflow (11), express (10), java (9), swift (8), flask (7), keras (7), plus ios/android as platform keys (12 each). Truncated professor names were completed for the cfd0b8e person importer (bare Paitoon/Dr. Benjawan/Mr. Chayapol forms now match their full names). sp-1628 and sp-1800 remain advisorless (verified against full documents); sp-1628 has no platform. Agent PDF reading on this machine requires poppler (`brew install poppler`); without it the Read tool fails on some PDFs and agents improvise CPU-heavy parsers. Remaining unreviewed: 72 projects. Extractor commits: 466f1f8, b6ee8c7.
