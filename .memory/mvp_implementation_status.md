...
8. Artifact storage must become a pluggable interface with a per-Artifact backend marker. Decided in AD-011 (2026-09-10, supersedes AD-008, not yet implemented). VM-local primary storage and S3-as-a-service were rejected by the institution's other admin; Google Drive was evaluated and rejected (consumer-account lock exposure, OAuth refresh-token fragility, per-user API rate limits, no institutional second copy). Provider selected: Backblaze B2. Reasons: no card required, first 10 GB always free, free egress up to 3x average monthly storage is sufficient at current scale and grows with the corpus, and R2's unmetered-egress advantage is unusable at this scale while requiring a payment method on file. Both speak the S3 API, so moving providers later is configuration plus a data migration. Serving always proxies through the API so visitors never contact the storage provider. Interface extraction requires `Open` to stop returning `*os.File` (`io.ReadSeekCloser` plus size, since public serving needs byte ranges). Provisioning done 2026-09-10: the B2 account, a bucket-scoped key named `ause-discovery-app-api`, and endpoint `s3.us-west-004.backblazeb2.com` are ready. The endpoint and key ID are recorded publicly in `.env.example` (maintainer judged them non-sensitive). The application key stays only in the deployment environment file. The bucket is `ause-discovery-artifacts` and a full round trip (put, checksum-verified read, delete, 404 on re-read) passed 2026-09-10. The put response confirms AES256 server-side encryption. The key carries no list capability, which is acceptable because the backend addresses objects by exact key. De-risking probes 2026-09-10: the S3 endpoint honors `Range` headers correctly (`bytes 10-15/36` content range verified), so ranged public serving works, and the current aws-sdk-go-v2 pin candidates are core v1.47.0, config v1.33.4, credentials v1.20.4, and service/s3 v1.113.0. Environment variable names are reserved in `.env.example` (`AUSE_ARTIFACT_STORAGE_BACKEND`, `AUSE_ARTIFACT_B2_ENDPOINT`, `AUSE_ARTIFACT_B2_BUCKET`, `AUSE_ARTIFACT_B2_KEY_ID`, `AUSE_ARTIFACT_B2_APPLICATION_KEY`) and no code consumes them yet.
